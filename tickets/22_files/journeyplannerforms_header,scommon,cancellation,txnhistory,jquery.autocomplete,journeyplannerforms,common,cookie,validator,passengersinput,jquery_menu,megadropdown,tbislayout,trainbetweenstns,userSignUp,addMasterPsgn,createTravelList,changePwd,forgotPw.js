...
function deleteCookie(c_n){setCookie(c_n,"",-1);}      	var SrctznAge;
		...
		*/
		...
		var mobileRegEx = new RegExp("^[1-9]{1}[0-9]{9}$");
		var mobileInternationlRegEx = new RegExp("^[0-9]{4,15}$");
...
function chkBedRollSrctzn(){
	...
	if(seniorCitizenApplicable && $(".psgn-concopt:enabled:checked").length > 0 && forgoConcession && $(".psgn-forgoconopt:enabled:checked").length <= 0){
		if(confirm(formatMessage(validatorMsg["opt_forgoconcession"]))){
			return false;
		}
	}
	return true;
				...
				curtr.find(".psgn-disablePsgnType").bind("change",function(){
					...
					if(_nationalityinputflag){
						if(dpt=="" || dpt == -1){
							...
							curtr.find(".psgn-concopt").attr("checked",false).trigger("change");
							if(dpt != 2){
...
function initPassengerDetails(){
	...
	if(_clusterOnwardBooking || _planReturn){
		...
		$(".psgn-concopt,.psgn-forgoconopt,.psgn-bedRollChoice").attr("onfocus","loseFocus(this);").addClass("readonly");
