...
function deleteCookie(c_n){setCookie(c_n,"",-1);}      	var SrctznAge;
		...
		*/
		...
		var psgnFemaleLbl;
		var psgnDynGenderId;
		var psgnDynGenderClass;
		var securityinput;
		var randomidCheck1;
		var randomidCheck2;
		var randomidCheck3;
		var passportChoice = false;
$(document).ready(function(){
...
function validatePassengerForm(event,msgs){
	...
	passportChoice = false;
	$(".psgn-name").parent("td").parent("tr").each(function(index){
		var curtr = $(this);
		var ageIsValid = false;
		var psgnName = curtr.find(".psgn-name");
		curtr.find(".psgn-name,.psgn-age,."+psgnDynGenderClass+",.psgn-foodChoice,.psgn-idcardtype,.psgn-idcardnumber,.psgn-disablePsgnType,.psgn-disableCardId,.psgn-disablePsgnDob,.psgn-disableCardExpDt,.psgn-chilldBerth,.psgn-nationality,.psgn-concopt").removeClass("error_field");
		if($.trim(psgnName.val()) != ''){
			...
			if(!validAge(age.val())){
				...
				var gender = curtr.find("."+psgnDynGenderClass+"");
				if(!(gender.find("option:selected").text() == psgnMaleLbl || gender.find("option:selected").text() == psgnFemaleLbl || gender.val() == "T") && securityCodeDay==0){
					msgs.push(formatMessage(validatorMsg["no_contiguous_psgn_input"],index+1));
					gender.addClass("error_field");
				}else{
					if(ageIsValid && gender.find("option:selected").text() == psgnMaleLbl && age.val() > maxChildAge )
						adultMaleExists = true;
					gender.removeClass("error_field");
				}
				
				if(selectedQuotaValue=="LD" && gender.find("option:selected").text() == psgnMaleLbl && age.val()>11){
				...
				if(!(gender.find("option:selected").text() == psgnMaleLbl && age.val() >=60 || gender.find("option:selected").text() == psgnFemaleLbl && age.val() >=45)){
			...
			var age = curtr.find(".psgn-age");
			var gender = curtr.find("."+psgnDynGenderClass+"");
			if($.trim(age.val()) != '' || (gender.find("option:selected").text() == psgnMaleLbl || gender.find("option:selected").text() == psgnFemaleLbl)){
				psgnName.addClass("error_field");
				if(!validAge(age.val())){
					age.addClass("error_field");
				}else{
				...
				if(!(gender.find("option:selected").text() == psgnMaleLbl || gender.find("option:selected").text() == psgnFemaleLbl)){
					gender.addClass("error_field");
				}else{
					gender.removeClass("error_field");
				}
	...
	if(numberOfPassengers != 2 || journeyClass == '2S' || journeyClass == 'FC' || journeyClass =='CC' || journeyClass == '1A' || journeyClass =='EC' || bkgClassCode == 'EA'){
		_twoSSEligible = false;
	}
	if(_concessionBooking){
...
function seniorCitizen(age,gender){
	if(gender == psgnMaleLbl && age >= SrctznAge ) return true;
	else if(gender == psgnFemaleLbl && age >= SrctnwAge ) return true;
	else return false;	
}
...
function selectMasterPassenger(){
	...
	for(i=existingPsgnCount,selectpsgn=0;  selectpsgn < selectedPsgns.length && i < maxPassengers; i++){
			...
			SetSelectItem("addPassengerForm\\:psdetail\\:"+i+"\\:"+psgnDynGenderId+"",$("#addPassengerForm\\:passengerlist\\:"+selectedPsgns[selectpsgn]+"\\:mstPsgnSexH").val());
			$("#addPassengerForm\\:psdetail\\:"+i+"\\:"+psgnDynGenderId+"").trigger("change");
			...
			if(psgnUidFlag == 2){
				if($("#addPassengerForm\\:psdetail\\:"+i+"\\:idCardType").length == 1){
					if($("#addPassengerForm\\:psdetail\\:"+i+"\\:idCardType").val() == "UNIQUE_ICARD"){
						...
						$("."+psgnDynGenderClass+":eq("+i+"),.psgn-idcardtype:eq("+i+"),.psgn-nationality:eq("+i+")").attr("onfocus","loseFocus(this);").addClass("readonly");					
					}else{
						$(".psgn-name:eq("+i+"),.psgn-age:eq("+i+"),.psgn-idcardnumber:eq("+i+")").removeAttr("readonly").removeAttr("onfocus").removeClass("readonly");
						$("."+psgnDynGenderClass+":eq("+i+"),.psgn-idcardtype:eq("+i+"),.psgn-nationality:eq("+i+")").removeAttr("onfocus").removeClass("readonly");
					}
				}	
			}
			selectpsgn++;
	...
	if(psgnUidFlag == 2 && i != maxPassengers){
		...
		$("."+psgnDynGenderClass+":gt("+i+"),.psgn-idcardtype:gt("+i+")").removeAttr("onfocus").removeClass("readonly");
	}
...
function resetPsgnDetails(fromIndex, toIndex){
	...
	for(var i=fromIndex;  i  < toIndex &&  i<psgnArray.length; i++){
			$(psgnArray[i]).val("");
			$("#addPassengerForm\\:psdetail\\:"+i+"\\:"+psgnDynGenderId+" option:eq(0)").attr("selected","selected");
			$("#addPassengerForm\\:psdetail\\:"+i+"\\:"+psgnDynGenderId+"").trigger("change");
...
function selectTravelListPassengers(){
	...
	for(i=0,selectpsgn=0;  selectpsgn < numTravelListPsgn && i < maxPassengers; i++){
			...
			SetSelectItem("addPassengerForm\\:psdetail\\:"+i+"\\:"+psgnDynGenderId+"",$("#addPassengerForm\\:travelList\\:"+selectpsgn+"\\:tlPsgnSexH").val());
			$("#addPassengerForm\\:psdetail\\:"+i+"\\:"+psgnDynGenderId+"").trigger("change");
...
function disablePsgnInput(){
	if(uidMandatory && psgnUidFlag == 1){
		...
		$("."+psgnDynGenderClass+",.psgn-idcardtype").attr("onfocus","loseFocus(this);").addClass("readonly");
	}
}
...
function setSecurityParam(){
	if(securityCodeDay==0){
		psgnFemaleLbl='Female';
		psgnMaleLbl='Male';
	}
}
function disableChildBerthOpt() {
	$(".psgn-name").parent("td").parent("tr").each(function(index){
		if(childBerthFlag1) {
			...
			if(childage.val()=='' || validAdultAge(Number(childage.val())) || (bkgClassCode =='2S' || bkgClassCode =='CC' || bkgClassCode =='EC' || bkgClassCode == 'FC' || bkgClassCode == 'EA') || (selectedQuotaValue == 'PT') || suvidhaFlag || trainECFlag) {
				curtr.find(".psgn-chilldBerth").attr("checked",true).attr("disabled","disabled");
			}
			childage.bind("change",function(){
			    if(validAge(this.value) && validChildAge(this.value) && !(bkgClassCode =='2S' || bkgClassCode =='CC' || bkgClassCode =='EC' || bkgClassCode == 'FC' || bkgClassCode == 'EA') 
...
function disableConcessionOpt(){
	if(seniorCitizenApplicable || _concessionBooking){		
		$(".psgn-name").parent("td").parent("tr").each(function(index){
			...
			var age=curtr.find(".psgn-age");
			var gender=curtr.find("."+psgnDynGenderClass+"");
			if(!(validAge(age.val()) && seniorCitizen(age.val(),gender.find("option:selected").text()))){
				curtr.find(".psgn-concopt").val("0").attr("disabled","disabled").trigger("change");
			}
			age.bind("change",function(){
			    if(validAge(this.value) && seniorCitizen(this.value,$(this).parent("td").parent("tr").find("."+psgnDynGenderClass+"").find("option:selected").text()) && 
			    		$(this).parent("td").parent("tr").find(".psgn-nationality").val()=='IN'){
					curtr.find(".psgn-concopt").removeAttr("disabled");
					if(_concessionBooking){
						curtr.find(".psgn-disablePsgnType").trigger("change");
					}
				}else{
			...
			gender.bind("change",function(){
				if(validAge($(this).parent("td").parent("tr").find(".psgn-age").val()) && seniorCitizen($(this).parent("td").parent("tr").find(".psgn-age").val(),$(this).parent("td").parent("tr").find("."+psgnDynGenderClass+"").find("option:selected").text())
						&& $(this).parent("td").parent("tr").find(".psgn-nationality").val()=='IN'){
					curtr.find(".psgn-concopt").removeAttr("disabled");
					if(_concessionBooking){
						curtr.find(".psgn-disablePsgnType").trigger("change");
			...
			curtr.find(".psgn-concopt").bind("change",function(){
				disableAutoUpgrade();
				if(($(this).val()== 1 || $(this).val()== 2) && _concessionBooking ){
					...
					if(_nationalityinputflag){
						if(dpt=="" || dpt == -1){
							...
							if(curtr.find(".psgn-nationality").val()=='IN'){
								...
								curtr.find(".psgn-age").trigger("change");
							}
							else{
	...
	if(_nationalityinputflag){
		...
		$(".psgn-name").parent("td").parent("tr").each(function(index){
			...
			//if($("#addPassengerForm_errorloc").is(':hidden')){
				for(var i=0; i < maxPassengers; i++){
					if(selectedQuotaValue == 'FT'){
						...
						$("#addPassengerForm\\:psdetail\\:"+i+"\\:idCardType").val("PASSPORT").attr("onfocus","loseFocus(this);").prop("disabled",true);
						$("#addPassengerForm\\:psdetail\\:"+i+"\\:idCardNumber").val("").removeAttr("readonly").removeClass("readonly").attr('maxlength',maxpassportLength);
					}else{
			...
			$(".psgn-name").parent("td").parent("tr").each(function(index){
				var curtr=$(this);
				if(_concessionBooking || seniorCitizenApplicable){
					if(validAge(curtr.find(".psgn-age").val()) && seniorCitizen(curtr.find(".psgn-age").val(),curtr.find("."+psgnDynGenderClass+"").find("option:selected").text()) && 
							curtr.find(".psgn-nationality").val()=='IN'){
						curtr.find(".psgn-concopt").removeAttr("disabled");
					}else{
				    	curtr.find(".psgn-concopt").val("0").attr("disabled","disabled").trigger("change");
...
function initPassengerDetails(){
	...
	if(_clusterOnwardBooking || _planReturn){
		...
		$(".psgn-nationality,.psgn-idcardtype,."+psgnDynGenderClass+",.psgn-disablePsgnType").attr("readonly","readonly").attr("onfocus","loseFocus(this);").prop("disabled",true);
	}
	errorexist=false;
}
 ...
 if(upaschecklower.test(newPassword.val()))
  {
    ...
    if(lastName.val() != '' )  {
        ...
        if (onlyalphaSpace.test(lastName.val()))
        {
        }
        else{
    ...
    if($("#userRegistrationForm\\:gender\\:0:checked").length ==0 && $("#userRegistrationForm\\:gender\\:1:checked").length ==0 && $("#userRegistrationForm\\:gender\\:2:checked").length ==0) {
...
function validateProfile() {
    ...
    var pancardMandateFlg = $("#updateUserDtls\\:pancardMandateFlgProfile"); 
       if(secAnswer.val() == '' )  {
        ...
        if($("#updateUserDtls\\:genderProfile\\:0:checked").length ==0 && $("#updateUserDtls\\:genderProfile\\:1:checked").length ==0 && $("#updateUserDtls\\:genderProfile\\:2:checked").length ==0) {
